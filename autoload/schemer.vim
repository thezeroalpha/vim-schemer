...
function! s:ColorInit(...) "{{{1
  ...
  let s:stop = 0
  let s:use_virtual_text = has("nvim") && get(g:, 'schemer_use_virtual_text', 0)
  ...
  " Enable Autocommands
  if exists("g:schemer_auto_color")
    call s:AutoCmds(g:schemer_auto_color)
  endif
  ...
  " other filetypes
  let s:schemer_term_disable = !(empty(&filetype) || &filetype != 'text')
  let s:schemer_nroff_disable = !(empty(&filetype) || &filetype != 'text')

  " Debugging
  let s:debug = get(g:, 'schemer_debug', 0)

  " Don't highlight comment?
  let s:skip_comments = get(g:, 'schemer_skip_comments', 0)
  ...
  if !exists('g:schemer_fgcontrast')
    " Default to black / white
    let g:schemer_fgcontrast = len(s:predefined_fgcolors['dark']) - 1
  elseif g:schemer_fgcontrast >= len(s:predefined_fgcolors['dark'])
    call s:Warn("g:schemer_fgcontrast value invalid, using default")
    let g:schemer_fgcontrast = len(s:predefined_fgcolors['dark']) - 1
  endif
  ...
  if !exists("s:old_fgcontrast")
    ...
    let s:old_fgcontrast = g:schemer_fgcontrast
  endif

  if exists("g:schemer_swap_fgbg")
    if s:swap_fg_bg != g:schemer_swap_fgbg
      let s:force_hl = 1
    endif
    let s:swap_fg_bg = g:schemer_swap_fgbg
  endif

  if exists("g:schemer_colornames")
    if exists("s:color_names") &&
          \ s:color_names != g:schemer_colornames
      let s:force_hl = 1
    endif
    let s:color_names = g:schemer_colornames
  else
    let s:color_names = 1
  endif

  let s:color_syntax = get(g:, 'schemer_syntax', 0)
  if get(g:, 'schemer_only_unfolded', 0) && exists(":foldd") == 1
  ...
  if !s:force_hl && s:old_fgcontrast != g:schemer_fgcontrast
        \ && s:swap_fg_bg == 0
  ...
  let s:hex_pattern = get(g:, 'schemer_hex_pattern',
        \ ['#', '\%(\x\{3}\|\x\{6}\|\x\{8\}\)', '\%(\>\|[-_]\)\@='])
  ...
  if s:HasGui() || &t_Co >= 8 || s:HasColorPattern()
    ...
    if &t_Co > 16 || s:HasGui()
      let s:colors = (exists("g:schemer_x11_names") ?
    ...
    if exists("g:schemer_custom_colors")
      call extend(s:colors, g:schemer_custom_colors, 'force')
    endif
  ...
  let s:color_patterns = {
        \ 'rgb': ['rgb(\s*\%(\d\+%\?[^)]*\)\{3})',
        \ function("s:ColorRGBValues"), 'schemer_rgb', 1, [] ],
        \ 'rgba': ['rgba(\s*\%(\d\+%\?\D*\)\{3}\%(\%(0\?\%(.\d\+\)\?\)\|1\))',
        \ function("s:ColorRGBValues"), 'schemer_rgba', 1, [] ],
        \ 'hsla': ['hsla\=(\s*\%(\d\+%\?\D*\)\{3}\%(\%(0\?\%(.\d\+\)\?\)\|1\)\=)',
        \ function("s:ColorHSLValues"), 'schemer_hsla', 1, [] ],
        \ 'vimcolors':  ['\%(gui[fb]g\|cterm[fb]g\)\s*=\s*\<\%(\d\+\|#\x\{6}\|\w\+\)\>',
        \ function("s:PreviewVimColors"), 'schemer_vimcolors', '&ft ==# "vim"', [] ],
        \ 'vimhighlight': ['^\s*\%(\%[Html]HiLink\s\+\w\+\s\+\w\+\)\|'.
        \ '\(^\s*hi\%[ghlight]!\?\s\+\(clear\)\@!\S\+.*\)',
        \ function("s:PreviewVimHighlight"), 'schemer_vimhighlight', '&ft ==# "vim"', [] ],
        \ 'taskwarrior':  ['^color[^=]*=\zs.\+$',
        \ function("s:PreviewTaskWarriorColors"), 'schemer_taskwarrior', 'expand("%:e") ==# "theme"', [] ],
        \ 'hex': [join(s:hex_pattern, ''), function("s:PreviewColorHex"), 'schemer_hex', 1, [] ],
        \ 'vimhighl_dump': ['^\v\w+\s+<xxx>%((\s+(term|cterm%([bf]g)?|gui%(%([bf]g|sp))?)\='.
        \ '[#0-9A-Za-z_,]+)+)?%(\_\s+links to \w+)?%( cleared)@!$',
        \ function("s:PreviewVimHighlightDump"), 'schemer_vimhighl_dump', 'empty(&ft)', [] ]
        \ }
...
function! s:AddOffset(list) "{{{1
  ...
  for val in a:list
    let val = ('0X'.val) + 0
    if val < get(g:, 'schemer_min_offset', 0)
      let val = get(g:, 'schemer_add_offset', 0)
    endif
    call add(result, val)
  endfor
  return result
...
function! s:SyntaxMatcher(enable) "{{{1
  ...
  if len(list) > 1000
    ...
    call s:Warn("Schemer many colors detected, syntax highlighting will probably slow down Vim considerably!")
  endif
  if &ft =~? 'css'
...
function! s:DoColor(force, line1, line2, ...) "{{{1
  ...
  try
    if v:version < 800 && !has('nvim')
      call s:Warn("Schemer needs Vim 8.0")
      return
    endif
  ...
  if &t_Co > 16 || s:HasGui()
    ...
    for Pat in values(s:color_patterns)
      ...
      if s:CheckTimeout(Pat[0], a:force) && !s:IsInComment()
        ...
        try
          if Pat[2] ==# 'schemer_vimhighlight' && !empty(bufname(''))
  ...
  if !exists("#FTSchemer#BufWinEnter#<buffer>") && empty(error)
    " Initialise current window.
    call s:LocalFTAutoCmds(1)
    call s:ColorWinEnter(1, 1) " don't call DoColor recursively!
  endif
...
function! s:AutoCmds(enable) "{{{1
  if a:enable && !get(g:, 'schemer_debug', 0)
    aug Schemer
      au!
      if get(g:, 'schemer_insertleave', 1)
        au InsertLeave *  sil call s:ColorLine('', line('w0'), line('w$'))
      endif
      if get(g:, 'schemer_textchangedi', 1)
        au TextChangedI * sil call s:ColorLine('', line('.'),line('.'))
      endif
      if get(g:, 'schemer_guienter', 1)
        au GUIEnter * sil call s:DoColor('!', 1, line('$'))
      endif
      if get(g:, 'schemer_colorscheme', 1)
        au ColorScheme * sil call s:DoColor('!', 1, line('$'))
      endif
      if get(g:, 'schemer_bufwinenter', 1)
        au BufWinEnter * sil call s:ColorWinEnter()
      endif
      if get(g:, 'schemer_winenter', 1)
        au WinEnter * sil call s:ColorWinEnter()
      endif
    aug END
  else
    aug Schemer
...
function! s:LocalFTAutoCmds(enable) "{{{1
  if a:enable
    aug FTSchemer
      au!
      if !exists("#Schemer#InsertLeave") && get(g:, 'schemer_insertleave', 1)
        " InsertLeave autocommand already exists
        au InsertLeave <buffer> silent call
              \ s:ColorLine('', line('w0'), line('w$'))
      endif
      if get(g:, 'schemer_cursormovedi', 1)
        au CursorMovedI <buffer> call s:ColorLine('',line('.'), line('.'))
      endif
      if get(g:, 'schemer_cursormoved', 1)
        au CursorMoved <buffer> call s:ColorLine('',line('.'), line('.'))
      endif
      if !exists("#Schemer#BufWinEnter") && get(g:, 'schemer_bufwinenter', 1)
        au BufWinEnter <buffer> silent call s:ColorWinEnter()
      endif
      if !exists("#Schemer#WinEnter") && get(g:, 'schemer_winenter', 1)
        au WinEnter <buffer> silent call s:ColorWinEnter()
      endif
      " disables colorizing on switching buffers inside a single window
      if get(g:, 'schemer_bufleave', 1)
        au BufLeave <buffer> if !get(g:, 'schemer_disable_bufleave', 0) | call s:ColorOff() |endif
      endif
      if !exists("#Schemer#GUIEnter") && get(g:, 'schemer_guienter', 1)
        au GUIEnter <buffer> silent call s:DoColor('!', 1, line('$'))
      endif
      if !exists("#Schemer#ColorScheme") && get(g:, 'schemer_colorscheme', 1)
        au ColorScheme <buffer> silent call s:DoColor('!', 1, line('$'))
      endif
    aug END
    if !exists("b:undo_ftplugin")
      " simply unlet a dummy variable
      let b:undo_ftplugin = 'unlet! b:Schemer_foobar'
    endif
    " Delete specific auto commands, because the filetype
    " has been changed.
    let b:undo_ftplugin .= '| exe "sil! au! FTSchemer"'
    let b:undo_ftplugin .= '| exe "sil! aug! FTSchemer"'
    let b:undo_ftplugin .= '| exe ":call s:ColorOff()"'
  else
    aug FTSchemer
      au!
    aug END
...
function! s:ColorWinEnter(...) "{{{1
  ...
  " be fast!
  if !force
    let ft_list = split(get(g:, "schemer_auto_filetype", ""), ',')
    if match(ft_list, "^".&ft."$") == -1
      " current filetype doesn't match g:schemer_auto_filetype,
      " so nothing to do
      return
    endif
    if get(b:, 'Schemer_changedtick', 0) == b:changedtick &&
          \ !empty(getmatches())
      " nothing to do
      return
    endif
  endif
  let g:schemer_only_unfolded = 1
  let _c = getpos('.')
  if !exists("a:2")
    " don't call it recursively!
    call s:DoColor('', 1, line('$'))
  endif
  let b:Schemer_changedtick = b:changedtick
  unlet! g:schemer_only_unfolded
  call setpos('.', _c)
endfu
...
function! s:ColorLine(force, start, end) "{{{1
  if get(b:, 'Schemer_changedtick', 0) == b:changedtick && empty(a:force)
    ...
    call s:DoColor(a:force, a:start, a:end)
    let b:Schemer_changedtick = b:changedtick
  endif
endfu

function! Schemer#ProcessFile(file) abort
  let primary = []
  let links = {}
  ...
  while cursor < nlines
    ...
    if line =~ "^palette:"
      while 1
        ...
        if entry[1] !~ "\\.$"
          let entry[1] = entry[1]->substitute(",", "", "g")
          let colornames[entry[0]] = entry[1]
        else
          let entry[1] = entry[1]->substitute("\\.", "", "g")
          let colornames[entry[0]] = entry[1]
          break
    ...
    elseif line->empty() || line =~ "^\""
      " Do nothing
    elseif line =~ "^background"
      let background = line->split()[-1]
    else
      let line_arr = line->split()
      let group = line_arr[0]
      let rest = line_arr[1:-1]->join(' ')

      let rest_split = rest->split('\.')
      let colors = rest_split[0]->split(',')->map('trim(v:val)')

      let fg = colors[0]
      let fg = (fg !~ '^#' && fg !=# 'NONE') ? colornames[colors[0]] : colors[0]

      if len(colors) ==# 2 && !empty(colors[1]) && colors[1] !=# 'NONE'
        let bg = (colors[1] =~ "^#") ? colors[1] : colornames[colors[1]]
      else
        let bg = 'NONE'
      endif

      if len(rest_split) ==# 2
        let attrs = rest_split[1]->split(',')->map('trim(v:val)')->join(',')
      else
        let attrs = 'NONE'
      endif

      let primary = primary->add([group, fg, bg, attrs])
    endif
